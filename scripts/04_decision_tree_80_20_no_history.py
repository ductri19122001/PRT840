...
def load_modeling_dataframe_with_capture_dates() -> pd.DataFrame:
    ...
    metadata_dataframe = pd.read_csv(METADATA_FILE,usecols=[ROW_ID_COLUMN, CAPTURE_DATE_COLUMN, DETAILED_LABEL_COLUMN])
...
def save_attack_type_results(
    ...
    results = pd.DataFrame(
        {
            "detailedlabel": test_dataframe[DETAILED_LABEL_COLUMN]
            .fillna("Unknown")
            .astype(str)
            .to_numpy(),
            "true_label": y_true.to_numpy(),
    ...
    malicious = results.loc[
        results["true_label"] == 1
    ].copy()

    summary = (
        malicious.groupby("detailedlabel")
        .agg(
            test_samples=("true_label", "size"),
            detected=("predicted_label", lambda x: int((x == 1).sum())),
            missed=("predicted_label", lambda x: int((x == 0).sum())),
        )
        .reset_index()
        .rename(columns={"detailedlabel": "attack_type"})
    )

    summary["recall"] = summary["detected"] / summary["test_samples"]
    summary["recall_percent"] = summary["recall"] * 100
...
def main() -> None:
    ...
    attack_type_results = save_attack_type_results(
    ...
    print(
        attack_type_results.to_string(
            index=False,
            formatters={
                "recall": "{:.4f}".format,
                "recall_percent": "{:.2f}%".format,
            },
        )
    )

    print(f"\nSaved attack-type results to: {ATTACK_TYPE_OUTPUT_FILE}")
    print("\nEvaluation metrics:")
